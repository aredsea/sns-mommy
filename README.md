...
- **1단계 (현재, v1.0.3): 패키징 머지 + UX 수정 + SNS 연동 강화**
  - 1.0.3: 줌 갤러리 SNS 레퍼런스 **근본 수정** — `srimg:` 채팅 이미지를 placeholder SVG 우회하고 `readSrImageData`로 store에서 직접 바이트 읽기. 썸네일 선택 시 **녹색 보더 + 체크마크 뱃지**로 명확하게 표시. DM 라우팅 **단일 트랙 통일** — X·IG 모두 카카오톡(DM) 화면으로, **profile.pkey 기준 1방**(여러 계정도 같은 캐릭터로 라우팅), 방 타이틀 = **캐릭터 본명**(profile.realname). DM 입력바 **flex-wrap 허용** + input min-width 60% + UI-only 도구버튼(메시지도구·텍스트첨부) 숨김으로 타이핑 영역 가시화.
  - 1.0.2: 줌 갤러리 SNS 레퍼런스 — **인물(프로필) 선택 모달** 후 지정(시뮬 봇 대응), pkey별 레퍼런스 맵으로 저장(레거시 하위호환), SNS 이미지 생성 시 작성자 pkey에 맞는 레퍼런스+태그 자동 결합. "메신저"→"DM" 표기. DM 입력바 모바일 정상화 1차. X 팔로잉 시 "DM" 버튼 노출, IG "메시지 보내기" 라우팅.
